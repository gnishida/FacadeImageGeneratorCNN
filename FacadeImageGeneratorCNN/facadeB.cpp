...
cv::Mat generateFacadeB(int width, int height, int thickness, const std::vector<float>& params) {
	...
	return generateFacadeB(NF, NC, width, height, thickness, WW, WH, WS, WT, WB, BS, TW, AH, FH, BH, 0, false);
}
...
cv::Mat generateFacadeB(int width, int height, int thickness, std::vector<float>& params, int window_displacement, bool noise, int edge_displacement, float window_prob, float edge_prob) {
	...
	return generateFacadeB(NF, NC, width, height, thickness, WW, WH, WS, WT, WB, BS, TW, AH, FH, BH, window_displacement, noise, edge_displacement, window_prob, edge_prob);
}

cv::Mat generateFacadeB(int NF, int NC, int width, int height, int thickness, float WW, float WH, float WS, float WT, float WB, float BS, float TW, float AH, float FH, float BH, int window_displacement, bool noise, int edge_displacement, float window_prob, float edge_prob) {
