...
cv::Mat generateFacadeD(int width, int height, int thickness, const std::vector<float>& params) {
	...
	std::cout << "params[13]: " << params[13] << ", params[14]: " << params[14] << ", DW: " << DW << ", DS: " << DS << std::endl;

	return generateFacadeD(NF, NC, width, height, thickness, WW, WH, WS, WT, WB, BS, TW, AH, FH, GH, ND, DT, DH, DW, DS, 0, false);
}
...
cv::Mat generateFacadeD(int width, int height, int thickness, std::vector<float>& params, int window_displacement, bool noise, int edge_displacement, float window_prob, float edge_prob) {
	...
	return generateFacadeD(NF, NC, width, height, thickness, WW, WH, WS, WT, WB, BS, TW, AH, FH, GH, ND, DT, DH, DW, DS, window_displacement, noise, edge_displacement, window_prob, edge_prob);
}

cv::Mat generateFacadeD(int NF, int NC, int width, int height, int thickness, float WW, float WH, float WS, float WT, float WB, float BS, float TW, float AH, float FH, float GH, int ND, float DT, float DH, float DW, float DS, int window_displacement, bool noise, int edge_displacement, float window_prob, float edge_prob) {
